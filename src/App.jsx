...
import { 
  ...
  UserPlus, Eye, Check, Database, Settings
} from 'lucide-react';
...
// Trage hier deine Daten aus der Firebase Console (Projekteinstellungen -> Web App) ein!
// ====================================================================================
...
const isConfigured = isPreviewEnvironment || (firebaseConfig.apiKey !== "DEIN_API_KEY" && firebaseConfig.apiKey !== undefined);
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsubUsers = onSnapshot(usersRef, (snap) => setUsers(snap.docs.map(d => d.data())), err => console.error(err));
    const unsubEvents = onSnapshot(eventsRef, (snap) => setEvents(snap.docs.map(d => d.data())), err => console.error(err));
