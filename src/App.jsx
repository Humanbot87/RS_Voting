      ...
      <header className="bg-gray-900 border-b border-gray-800 sticky top-0 z-10 shadow-md">
        <div className="max-w-5xl mx-auto px-4 py-4 flex justify-between items-center">
          ...
          <div className="flex items-center gap-4">
            ...
            <button onClick={handleLogout} className="p-2 hover:bg-gray-800 rounded-full transition-colors text-gray-400 hover:text-orange-500" title="Abmelden"><LogOut size={20} /></button>
          </div>
        </div>
      </header>
...
function LoginScreen({ onLogin, users, activeSessions, onSeed, isSeeding, db, appId, deobfuscate, obfuscate }) {
  ...
  const checkName = (e) => {
    ...
    if (session && (Date.now() - session.lastSeen < 60000)) return alert("Dieses Mitglied ist bereits an einem anderen Gerät angemeldet.");
      ...
      <div className="max-w-md w-full bg-gray-900 border border-gray-800 rounded-3xl p-8 shadow-2xl overflow-hidden relative text-center">
        ...
        {users.length === 0 ? (
          <div className="py-6">
            ...
            <button onClick={onSeed} disabled={isSeeding} className="w-full bg-orange-500 hover:bg-orange-600 text-gray-950 font-bold py-3 rounded-xl mt-4 uppercase text-xs">Vereinsdaten laden</button>
          </div>
        ) : (
          <>
            {step === 'name' && (
              <form onSubmit={checkName} className="space-y-4 text-left">
                <div>
                   <label className="text-[10px] font-bold text-gray-500 uppercase ml-1 tracking-widest">Vorname</label>
                   <input type="text" required value={firstName} onChange={e => setFirstName(e.target.value)} placeholder="Max" className="w-full bg-gray-950 border border-gray-800 rounded-2xl px-5 py-4 text-white focus:outline-none focus:border-orange-500 transition-colors font-bold" />
                </div>
                <div>
                   <label className="text-[10px] font-bold text-gray-500 uppercase ml-1 tracking-widest">Nachname</label>
                   <input type="text" required value={lastName} onChange={e => setLastName(e.target.value)} placeholder="Muster" className="w-full bg-gray-950 border border-gray-800 rounded-2xl px-5 py-4 text-white focus:outline-none focus:border-orange-500 transition-colors font-bold" />
                </div>
                <button type="submit" className="w-full bg-orange-500 hover:bg-orange-600 text-gray-950 font-bold py-4 rounded-2xl mt-4 uppercase text-xs tracking-widest">Anmelden</button>
              </form>
            )}

            {step === 'password' && (
              <form onSubmit={handlePasswordSubmit} className="space-y-4">
                <div className="flex items-center gap-3 mb-4 p-4 bg-orange-500/10 border border-orange-500/20 rounded-2xl text-orange-400 text-left">
                    <Lock size={20} /> <span className="text-xs font-bold uppercase tracking-wider">Vorstand Login</span>
                </div>
                <input type="password" required autoFocus value={password} onChange={e => setPassword(e.target.value)} placeholder="Vorstands-Passwort" className="w-full bg-gray-950 border border-gray-800 rounded-2xl px-5 py-4 text-white focus:outline-none focus:border-orange-500 transition-colors text-center tracking-widest font-bold" />
                <button type="submit" className="w-full bg-orange-500 hover:bg-orange-600 text-gray-950 font-bold py-4 rounded-2xl mt-4 uppercase text-xs tracking-widest">Entsperren</button>
            ...
            {step === 'setup' && (
              <form onSubmit={handleSetupSubmit} className="space-y-4 text-left">
                <div className="flex items-center gap-3 mb-4 p-4 bg-blue-500/10 border border-blue-500/20 rounded-2xl text-blue-400">
                    <ShieldAlert size={20} /> <div><span className="text-xs font-bold block uppercase tracking-wider">Passwort festlegen</span><span className="text-[10px] opacity-70 italic">Erstanmeldung als Vorstandmitglied.</span></div>
                </div>
                <input type="password" required autoFocus value={password} onChange={e => setPassword(e.target.value)} placeholder="Neues Passwort wählen" className="w-full bg-gray-950 border border-gray-800 rounded-2xl px-5 py-4 text-white focus:outline-none focus:border-orange-500 transition-colors text-center tracking-widest font-bold" />
                <button type="submit" className="w-full bg-orange-500 hover:bg-orange-600 text-gray-950 font-bold py-4 rounded-2xl mt-4 uppercase text-xs tracking-widest">Speichern & Login</button>
              </form>
            )}
          </>
        )}
    ...
    <div className="space-y-6">
      ...
      {minutes.length === 0 ? (<div className="text-center py-16 bg-gray-900/50 rounded-2xl border border-dashed border-gray-800"><FileText size={48} className="mx-auto text-gray-700 mb-4" /><p className="text-gray-500 font-bold uppercase tracking-widest text-xs italic">Noch keine Protokolle vorhanden.</p></div>) : (
        <div className="grid gap-4">{minutes.sort((a,b) => (b.date || '').localeCompare(a.date || '')).map(m => (
            <div key={m.id} className="bg-gray-900 border border-gray-800 p-5 rounded-2xl flex justify-between items-center group hover:border-orange-500/30 transition-all shadow-md">
              <div className="flex items-center gap-4">
                ...
                <div><h3 className="text-lg font-bold text-white">Sitzung vom {new Date(m.date).toLocaleDateString('de-CH')}</h3><p className="text-xs text-gray-500 uppercase font-black tracking-widest mt-1">Vorstandsprotokoll</p></div>
              </div>
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap justify-between items-center gap-4">
        ...
        <div className="flex gap-2">
            ...
            <button onClick={() => { setShowAdd(true); setEditingUser(null); }} className="bg-orange-500 hover:bg-orange-600 text-gray-950 font-bold px-4 py-2 rounded-lg flex items-center gap-2 transition-all shadow-lg active:scale-95"><UserPlus size={18} /> Hinzufügen</button>
        </div>
      </div>
...
function MemberForm({ onSubmit, initialData, onCancel }) {
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); onSubmit({ firstName: firstName.trim(), lastName: lastName.trim(), role, groups: selectedGroups, password: initialData?.password || "" }); }} className="bg-gray-900 border-2 border-orange-500/10 p-8 rounded-3xl mb-8 shadow-2xl relative overflow-hidden animate-in fade-in slide-in-from-top-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-6 border-t border-gray-800 pt-6 text-left">
